...
const fs = require('fs');
const {
  getExchangeRate, getTopShotPackInfo, getTopShotSearchActivity, getMomentRanksData, findMomentDetailsOfSerial,
} = require('./apiRequests');
const {
  today, getDaysHeld, timestamp, projectFolder, filesPath, activityFilesPath,
} = require('./common');
  ...
  df = df.map(
    (row) => {
      ...
      if (['P2P_MOMENT_LISTING_SOLD', 'OFFER_COMPLETED'].includes(row.get('activity_details'))) {
        ...
        } else if (!row.get('sale_profit_usd')) {
  ...
  df = df.map(
    (row) => {
      if (row.get('status') !== 'SUCCEEDED' || row.get('payment_method') === 'VISA') {
        ...
        accountBalance += Number(row.get('total_usd'));
...
function prepareDataForRowMr(activity, saleOrPurchase) {
  ...
  const { playCategory } = activity.moment;
  const { serialNumber } = activity;
  const orderId = null;
...
async function reconcileActivityWithExport(df, topShotActivityData, mrActivity, dapperID) {
  ...
  const noMatch = [];
  const usedActivityIdMatches = [];
  const matchedMrActivities = [];
